package com.diplomkz.decentkz.Admin;
...
public class NewProductActivity extends AppCompatActivity {
    String[] categoriesList = {"Выберите категорию", "Мужчинам", "Женщинам", "Детям",};
    String[] brandsList = {"Выберите бренд", "Decent", "Другое"};
    String[] sizeTypeList = {"Выберите размер", "XZ", "S", "M", "L", "XL", "XXL", "Oversize"};
    String[] sizeList = {"Select Size", "M", "XL", "S", "2XL",  "Oversize"};
    ...
    private void SettingClickListners() {
        ...
        uploadPhotoBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                startActivityForResult(Intent.createChooser(intent, "Выберите изобрежение"), PICK_IMAGE_REQUEST);
        ...
        addBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(filePath==null){
                    Toast.makeText(NewProductActivity.this, "Пожалуйста, выберите изобрежение", Toast.LENGTH_SHORT).show();
                }
                else if(TextUtils.isEmpty(name)){
                    nameEt.setError("Введите название товара");
                    nameEt.requestFocus();
                }
                else if(category.equals("Выберите категорию")){
                    Toast.makeText(NewProductActivity.this, "Выберите категорию", Toast.LENGTH_SHORT).show();
                }
                else if(brand.equals("Выберите имя бренда")){
                    Toast.makeText(NewProductActivity.this, "Выберите имя бренда", Toast.LENGTH_SHORT).show();
                }
                else if(sizeType.equals("Выберите тип размера")){
                    Toast.makeText(NewProductActivity.this, "Выберите тип размера", Toast.LENGTH_SHORT).show();
                }
                else if(TextUtils.isEmpty(price)){
                    priceEt.setError("Введите цену");
                    priceEt.requestFocus();
                }
                else if(TextUtils.isEmpty(color)){
                    colorEt.setError("Введите цвет");
                    colorEt.requestFocus();
                }
                else if(TextUtils.isEmpty(stock)){
                    stockEt.setError("Введите количество");
                    stockEt.requestFocus();
                }
                else if(TextUtils.isEmpty(desc)){
                    descriptionEt.setError("Введите описание");
        ...
        if (filePath != null) {
            ...
            uploadTask.addOnFailureListener(new OnFailureListener() {
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    Toast.makeText(NewProductActivity.this, "Фото загружается...", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(NewProductActivity.this, "Выберите фото", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void SaveInfoToDatabase() {
        ...
        myRootRef.child("Products").child(key).setValue(product).addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                progressBar.setVisibility(View.GONE);
                Toast.makeText(NewProductActivity.this, "Продукт добавлен", Toast.LENGTH_SHORT).show();
                finish();
